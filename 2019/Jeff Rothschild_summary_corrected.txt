...
• Using the commented code to create an IO redirect or for moving data between MS-DOS and a Unix system
...
• The speaker implemented a simple IP UDP protocol in assembler for a machine with only 64K bytes of RAM
...
• Explanation of EPSI DIC character encoding used by IBM in the past
• Honeywell's mainframe and GMOs operating system
...
• F-truncate operation added to support DOS behaviour on Unix
...
• The speaker discusses a Unix project that handled many keystrokes without issue.
• A specific function called truncate caused the speaker to introduce a bug in the kernel due to missing code paths.
...
• Tweet about UEFI reboot networking stack and DMA
...
• Co-locating game servers with internet service providers (PSI NET)
...
• Converting Quake to IP from Novel network protocol
...
• A specific problem mentioned was with teacake, a distributed hash table that needed to be improved for scalability and performance
• The solution involved making it multithreaded and reducing variability in request times
• Another problem mentioned was buffer overflow and packet drop due to TCP variable retries and congestion control
• Problems with teacake code and TCP protocol
...
• Experience with finding and resolving the teacake problem as one of the most frustrating and gratifying
...
• The potential of non-volatile RAM technologies (e.g. Intel's Octane)
• Historical parallels between past innovations (e.g. Multics) and current developments
• Database design and performance optimization, particularly in regard to memory usage
...
• Mention of YES going for two more minutes
